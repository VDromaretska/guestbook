...
app.get("/signatures", async (req, res) => {
  try {
    const signatures = await client.query("select * from signatures");
    res.status(201).json({
      status: "success",
      data: {
        signatures,
...
app.get("/signatures/:id", async (req, res) => {
  ...
  const value = [id]; // params are always string type
  try {
    const resObj = await client.query(
      "select * from signatures where id=$1",
      value
    );
    const signature = resObj.rows;
